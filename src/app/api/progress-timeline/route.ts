...
// GET /api/progress-timeline - Get progress timeline data
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'analytics') {
      ...
      const analytics = processTimelineAnalytics(progressReports, groupBy)
      ...
      // Handle regular timeline request
      const validation = progressTimelineQuerySchema.safeParse({
...
function processTimelineAnalytics(reports: any[], groupBy: string) {
  const analytics: any = {
    summary: {
  ...
  // Group by status
  analytics.byStatus = groupReportsByStatus(reports)
...
function groupReportsByStatus(reports: any[]) {
  const groups: any = {}
